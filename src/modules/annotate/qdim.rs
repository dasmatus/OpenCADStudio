...
use acadrust::entities::{
    Dimension, DimensionDiameter, DimensionLinear, DimensionOrdinate, DimensionRadius,
};
...
struct Candidate {
    point: DVec3,
    sources: Vec<DimensionAssociationSource>,
}
...
impl QdimCommand {
    pub fn new(selection: Vec<SelectionEntity>, dim_spacing: f64, snap_priority: u8) -> Self {
        ...
        Self {
            ...
            snap_priority: if snap_priority == 1 {
                SnapPriority::Intersections
            } else {
                SnapPriority::Endpoints
            },
    ...
    fn local_sources(&self) -> Vec<SourceCurve> {
        ...
        self.selection
            ...
            .filter_map(|selected| {
                ...
                crate::entities::curve::entity_curve_xy(&entity).map(|curve| SourceCurve {
    ...
    fn candidates(&self) -> Vec<Candidate> {
        ...
        if candidates.is_empty() {
            for source in &curves {
                for point in characteristic_points(&source.curve)
                    .into_iter()
                    .filter(|point| point.kind == SnapKind::Endpoint)
                {
        ...
        for candidate in candidates {
            if let Some(existing) = unique
                .iter_mut()
                .find(|existing| existing.point.distance(candidate.point) <= tolerance)
            {
                for source in candidate.sources {
                    if !existing
                        .sources
                        .iter()
                        .any(|current| current.handle == source.handle)
                    {
        ...
        unique.retain(|candidate| {
            !self
                .excluded
                .iter()
                .any(|point| point.distance(candidate.point) <= tolerance)
        });
        unique
    }
    ...
    fn radial_sources(&self) -> Vec<(Handle, Curve)> {
        self.local_sources()
            ...
            .map(|source| (source.handle, source.curve))
            .collect()
    }
    ...
    fn orientation(&self, points: &[Candidate], place: DVec3) -> bool {
        ...
        if (outside_x - outside_y).abs() > 1e-9 {
            outside_y >= outside_x
        } else {
    ...
    fn build_dimensions(&self, place_world: DVec3) -> Vec<(EntityType, DimensionAssociationInput)> {
        ...
        points.sort_by(|first, second| {
            if horizontal {
                first.point.x.total_cmp(&second.point.x)
            } else {
                first.point.y.total_cmp(&second.point.y)
            }
        });
        ...
        match self.mode {
            Mode::Continuous | Mode::Staggered => points
                .windows(2)
                .enumerate()
                .map(|(index, pair)| {
                    let offset = if self.mode == Mode::Staggered {
                        index as f64 * self.dim_spacing
                    } else {
                        0.0
                    };
                    let direction = if horizontal {
                        if place.y >= (pair[0].point.y + pair[1].point.y) * 0.5 {
                            1.0
                        } else {
                            -1.0
                        }
                    } else if place.x >= (pair[0].point.x + pair[1].point.x) * 0.5 {
                        1.0
                    } else {
                        -1.0
                    };
                    let shifted = if horizontal {
                        DVec3::new(place.x, place.y + direction * offset, 0.0)
                    } else {
                        DVec3::new(place.x + direction * offset, place.y, 0.0)
                    };
            ...
            Mode::Baseline => {
                ...
                points
                    ...
                    .map(|point| self.linear_dimension(&datum_candidate, point, place, horizontal))
                    .collect()
            }
            Mode::Ordinate => points
                .iter()
                .map(|point| self.ordinate_dimension(point, place, horizontal))
                .collect(),
    ...
    fn linear_dimension(
        ...
        place: DVec3,
        horizontal: bool,
    ) -> (EntityType, DimensionAssociationInput) {
        let mut dimension = DimensionLinear::new(v3(first.point), v3(second.point));
        dimension.rotation = if horizontal {
            0.0
        } else {
            std::f64::consts::FRAC_PI_2
        };
        let definition = if horizontal {
            DVec3::new(second.point.x, place.y, 0.0)
        } else {
            DVec3::new(place.x, second.point.y, 0.0)
        };
        let text = if horizontal {
            DVec3::new((first.point.x + second.point.x) * 0.5, place.y, 0.0)
        } else {
            DVec3::new(place.x, (first.point.y + second.point.y) * 0.5, 0.0)
        };
        ...
        let association = DimensionAssociationInput::Explicit(vec![
            first.sources.first().copied(),
            second.sources.first().copied(),
        ]);
        (
            self.plane
                .place_entity(EntityType::Dimension(Dimension::Linear(dimension))),
            association,
        )
    }

    fn ordinate_dimension(
        &self,
        point: &Candidate,
        place: DVec3,
        horizontal: bool,
        ...
        let leader = if horizontal {
            DVec3::new(point.point.x, place.y, 0.0)
        } else {
            DVec3::new(place.x, point.point.y, 0.0)
        };
        ...
        (
            self.plane
                .place_entity(EntityType::Dimension(Dimension::Ordinate(dimension))),
            DimensionAssociationInput::Explicit(vec![point.sources.first().copied()]),
        )
    }

    fn build_radial_dimensions(
        &self,
        place: DVec3,
    ) -> Vec<(EntityType, DimensionAssociationInput)> {
        self.radial_sources()
            .into_iter()
            .enumerate()
            .map(|(index, (handle, curve))| {
                ...
                direction.z = 0.0;
                direction = if direction.length_squared() <= f64::EPSILON {
                    DVec3::X
                } else {
                    direction.normalize()
                };
                ...
                let association = DimensionAssociationInput::Explicit(vec![Some(
                    DimensionAssociationSource::inferred(handle),
                )]);
    ...
    fn remove_candidate(&mut self, point_world: DVec3) {
        ...
        let Some(nearest) = all.iter().min_by(|first, second| {
            first
                .point
                .distance(point)
                .total_cmp(&second.point.distance(point))
        }) else {
            return;
        };
    ...
    fn preview_dimensions(&self, point: DVec3) -> Vec<WireModel> {
        self.build_dimensions(point)
            .into_iter()
            .enumerate()
            .flat_map(|(index, (entity, _))| preview_entity(index, &entity))
            .collect()
    }
}
...
impl CadCommand for QdimCommand {
    ...
    fn prompt(&self) -> String {
        match self.step {
            ...
            Step::Place => {
                t!("QDIM  Specify dimension line position or choose an option:").into_owned()
            }
            Step::Datum => t!("QDIM  Specify datum point:").into_owned(),
            Step::Edit => t!("QDIM  Select a point to remove (Enter when done):").into_owned(),
            Step::Settings => {
                t!("QDIM  Choose extension origin priority [Endpoints/Intersections]:").into_owned()
            }
        }
    }
    ...
    fn options(&self) -> Vec<CmdOption> {
        match self.step {
            Step::Place => vec![
                CmdOption::new("Continuous", "CONTINUOUS"),
                CmdOption::new("Staggered", "STAGGERED"),
                CmdOption::new("Baseline", "BASELINE"),
                CmdOption::new("Ordinate", "ORDINATE"),
                CmdOption::new("Radius", "RADIUS"),
                CmdOption::new("Diameter", "DIAMETER"),
                CmdOption::new("Datum point", "DATUM"),
                CmdOption::new("Edit", "EDIT"),
                CmdOption::new("Settings", "SETTINGS"),
            ],
            Step::Settings => vec![
                CmdOption::new("Endpoints", "ENDPOINTS"),
                CmdOption::new("Intersections", "INTERSECTIONS"),
            ],
            _ => Vec::new(),
        }
    }
    ...
    fn wants_text_input(&self) -> bool {
        matches!(self.step, Step::Place | Step::Settings)
    }
        ...
        if self.step == Step::Settings {
            match keyword.as_str() {
                "E" | "ENDPOINT" | "ENDPOINTS" => {
                    self.snap_priority = SnapPriority::Endpoints;
                    self.step = Step::Place;
                    return Some(CmdResult::SetQuickDimensionSnapPriority(0));
                }
                "I" | "INTERSECTION" | "INTERSECTIONS" => {
                    self.snap_priority = SnapPriority::Intersections;
                    self.step = Step::Place;
                    return Some(CmdResult::SetQuickDimensionSnapPriority(1));
                }
    ...
    fn on_point(&mut self, point: DVec3) -> CmdResult {
        match self.step {
            ...
            Step::Datum => {
                self.datum = Some(self.plane.to_local(point));
                self.step = Step::Place;
                CmdResult::NeedPoint
            }
            Step::Edit => {
                self.remove_candidate(point);
                CmdResult::NeedPoint
            }
            Step::Place => {
    ...
    fn on_enter(&mut self) -> CmdResult {
        match self.step {
            Step::Gathering if !self.selection.is_empty() => {
                self.step = Step::Place;
                CmdResult::NeedPoint
            }
            Step::Edit | Step::Datum | Step::Settings => {
                self.step = Step::Place;
                CmdResult::NeedPoint
            }
            _ => CmdResult::Cancel,
    ...
    fn on_preview_wires(&mut self, point: DVec3) -> Vec<WireModel> {
        if self.step == Step::Place {
            return self.preview_dimensions(point);
        }
        if self.step == Step::Edit {
            ...
            return self
                ...
                .flat_map(|(index, candidate)| {
                    ...
                    [
                        WireModel::solid_f64(
                            format!("qdim_edit_{index}_x"),
                            vec![x_start.to_array(), x_end.to_array()],
                            WireModel::CYAN,
                            false,
                        ),
                        WireModel::solid_f64(
                            format!("qdim_edit_{index}_y"),
                            vec![y_start.to_array(), y_end.to_array()],
                            WireModel::CYAN,
                            false,
                        ),
                    ]
...
fn candidate_tolerance(points: impl Iterator<Item = DVec3>) -> f64 {
    ...
    Tolerance::default()
        .linear()
        .max(magnitude * f64::EPSILON * 64.0)
}
...
fn dimension_reference_points(dimension: &Dimension) -> Vec<DVec3> {
    match dimension {
        ...
        Dimension::Angular2Ln(value) => vec![
            d3(value.first_point),
            d3(value.second_point),
            d3(value.angle_vertex),
            d3(value.definition_point),
        ],
        Dimension::Angular3Pt(value) => vec![
            d3(value.angle_vertex),
            d3(value.first_point),
            d3(value.second_point),
        ],
        Dimension::Arc(value) => vec![
            d3(value.center_point),
            d3(value.first_extension_point),
            d3(value.second_extension_point),
        ],
...
fn preview_entity(index: usize, entity: &EntityType) -> Vec<WireModel> {
    ...
    match dimension {
        Dimension::Linear(value) => {
            let first = d3(value.first_point);
            let second = d3(value.second_point);
            let definition = d3(value.definition_point);
            let horizontal = value.rotation.cos().abs() >= value.rotation.sin().abs();
            let first_line = if horizontal {
                DVec3::new(first.x, definition.y, definition.z)
            } else {
                DVec3::new(definition.x, first.y, definition.z)
            };
            let second_line = if horizontal {
                DVec3::new(second.x, definition.y, definition.z)
            } else {
                DVec3::new(definition.x, second.y, definition.z)
            };
        ...
        Dimension::Ordinate(value) => lines.push(vec![
            d3(value.feature_location).to_array(),
            d3(value.leader_endpoint).to_array(),
        ]),
        Dimension::Radius(value) => lines.push(vec![
            d3(value.angle_vertex).to_array(),
            d3(value.definition_point).to_array(),
            d3(value.base.text_middle_point).to_array(),
        ]),
        Dimension::Diameter(value) => lines.push(vec![
            d3(value.definition_point).to_array(),
            d3(value.angle_vertex).to_array(),
            d3(value.base.text_middle_point).to_array(),
        ]),
    ...
    lines
        ...
        .map(|(part, points)| {
            WireModel::solid_f64(
                format!("qdim_preview_{index}_{part}"),
                points,
                WireModel::CYAN,
                false,
            )
