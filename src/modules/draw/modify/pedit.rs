...
impl PeditCommand {
    ...
    pub fn with_preselection(mut self, handles: &[Handle]) -> Self {
        let multiple: Vec<_> = handles
            .iter()
            .copied()
            .filter(|handle| {
                self.info
                    .get(&handle.value())
                    .is_some_and(|info| info.mesh_size.is_none())
            })
            .collect();
        if multiple.len() > 1 {
            self.target = multiple.first().copied();
            self.mode = if multiple.iter().any(|handle| {
                self.info
                    .get(&handle.value())
                    .is_some_and(|info| info.convertible)
            }) {
    ...
    pub fn with_entities(mut self, entities: impl IntoIterator<Item = EntityType>) -> Self {
        self.entities = entities
            .into_iter()
            .map(|entity| (entity.common().handle.value(), entity))
            .collect();
        self
    }
        ...
        match result {
            Some(CmdResult::PeditOp { handle, op }) if !self.multiple.is_empty() => {
                Some(CmdResult::PeditOp {
                    handle,
                    op: PeditOp::Multiple(self.multiple.clone(), Box::new(op)),
                })
            }
            other => other,
        }
    ...
    fn mapped_tangents(&self, op: &PeditOp) -> Option<HashMap<usize, f64>> {
        ...
        match op {
            ...
            PeditOp::EditVertex {
                ...
                self.vertex_tangents
                    .iter()
                    .map(|(&old, &angle)| (if old > *index { old + 1 } else { old }, angle))
                    .collect(),
            ),
            PeditOp::VertexRange { first, last, split } => Some(
                self.vertex_tangents
                    .iter()
                    .filter_map(|(&index, &angle)| {
                        if *split {
                            if *first > 0 {
                                (index <= *first).then_some((index, angle))
                            } else {
                                (index >= *last).then_some((index.saturating_sub(*last), angle))
                            }
                            ...
                            Some((
                                if index >= *last {
                                    index.saturating_sub(last.saturating_sub(*first + 1))
                                } else {
                                    index
                                },
                                angle,
            ...
            PeditOp::Fit | PeditOp::FitWithTangents(_) | PeditOp::Decurve | PeditOp::Spline => {
    ...
    fn vertex_width(&self, index: usize) -> f64 {
        let entity = self
            .target
            .and_then(|handle| self.entities.get(&handle.value()));
        match entity {
            Some(EntityType::LwPolyline(p)) => {
                if p.constant_width != 0.0 {
                    p.constant_width
                } else {
                    p.vertices.get(index).map_or(0.0, |v| v.start_width)
                }
            }
            Some(EntityType::Polyline2D(p)) => p.vertices.get(index).map_or(p.start_width, |v| {
...
impl CadCommand for PeditCommand {
    ...
    fn options(&self) -> Vec<crate::command::CmdOption> {
        ...
        match &self.mode {
            ...
            .into_iter()
            .filter(|option| self.multiple.is_empty() || option.keyword != "E")
            .collect(),
            ...
            Mode::JoinType => vec![
                CmdOption::new("Extend", "E"),
                CmdOption::new("Add", "A"),
                CmdOption::new("Both", "B"),
            ],
            Mode::PolyVertex(_) => vec![
                CmdOption::new("Next", "N"),
                CmdOption::new("Previous", "P"),
                CmdOption::new("Break", "B"),
                CmdOption::new("Straighten", "S"),
                CmdOption::new("Tangent", "T"),
                CmdOption::new("Insert", "I"),
                CmdOption::new("Move", "M"),
    ...
    fn on_entity_replaced(&mut self, old: Handle, new_handles: &[Handle]) {
        ...
        if let Some(&nh) = new_handles.first() {
            ...
            for handle in &mut self.multiple {
                if *handle == old {
                    *handle = nh;
                }
            }
    ...
    fn inject_picked_entity(&mut self, entity: EntityType) {
        ...
        let count = self.vertex_count();
        if let Mode::PolyVertex(index) = &mut self.mode {
    ...
    fn on_pedit_applied(&mut self) {
        ...
        if let Some(mapped) = self.pending_tangents.take() {
            self.vertex_tangents = mapped;
        }
        self.undo_count = self.undo_count.saturating_add(1);
        self.multiple_history.push(
            self.pending_multiple
                .take()
                .unwrap_or_else(|| self.multiple.clone()),
        );
    ...
    fn wants_text_input(&self) -> bool {
        !matches!(
            self.mode,
            Mode::PickTarget | Mode::MeshVertexMove(_) | Mode::PolyMove(_) | Mode::PolyInsert(_)
        )
    }
    ...
    fn on_text_input(&mut self, text: &str) -> Option<CmdResult> {
        ...
        let current_width = if let Mode::PolyVertex(index) = self.mode {
            self.vertex_width(index)
        } else {
            0.0
        };
        let result = match &mut self.mode {
            Mode::PickTarget => {
                if matches!(up.as_str(), "M" | "MULTIPLE") {
                    self.mode = Mode::MultipleGather;
            ...
            Mode::JoinType => {
                self.join_type = match up.as_str() {
                    "E" | "EXTEND" => JoinType::Extend,
                    "A" | "ADD" => JoinType::Add,
                    "B" | "BOTH" => JoinType::Both,
                    _ => return None,
                };
            ...
            Mode::MultipleJoin => {
                if matches!(up.as_str(), "J" | "JOINTYPE") {
                    self.mode = Mode::JoinType;
                    return Some(CmdResult::NeedPoint);
                }
                let fuzz = text.trim().parse::<f64>().ok()?;
                if !fuzz.is_finite() || fuzz < 0.0 {
                    return Some(CmdResult::ReportError(
                        "PEDIT: fuzz distance must be nonnegative and finite.".to_string(),
                    ));
                }
                self.join_fuzz = fuzz;
                *JOIN_FUZZ.lock().unwrap_or_else(|error| error.into_inner()) = fuzz;
                ...
                return Some(CmdResult::PeditOp {
                    handle: self.target?,
                    op: PeditOp::JoinSelection(self.multiple.clone(), fuzz, self.join_type),
                });
            }
            Mode::MultipleConvert => match up.as_str() {
                "Y" | "YES" | "" => {
                    self.pending_multiple = Some(self.multiple.clone());
                    self.mode = Mode::Options;
                    Some(CmdResult::PeditOp {
                        handle: *self.multiple.first()?,
                        op: PeditOp::ConvertToPolyline,
                    })
                }
                "N" | "NO" => {
                    self.multiple.retain(|handle| {
                        self.info
                            .get(&handle.value())
                            .is_some_and(|info| info.is_poly)
                    });
            ...
            Mode::AwaitLinetype => {
                ...
                Some(CmdResult::PeditOp {
                    handle: self.target?,
                    op: PeditOp::SetLinetypeGeneration(enabled),
                })
            }
            ...
            Mode::PolyWidthEnd(index, start) => {
                ...
                Some(CmdResult::PeditOp {
                    handle: self.target?,
                    op: PeditOp::SetVertexWidth { index, start, end },
                })
            }
            ...
            Mode::PolyTangent(index) => {
                ...
                self.vertex_tangents
                    .insert(*index, angle.to_radians().rem_euclid(TAU));
                self.mode = Mode::PolyVertex(*index);
                Some(CmdResult::NeedPoint)
            }
            ...
            Mode::Options => {
                ...
                match up.as_str() {
                    "E" | "EDIT" if self.multiple.is_empty() => {
                        self.mode = Mode::PolyVertex(0);
                        Some(CmdResult::NeedPoint)
                    }
                    "R" | "REVERSE" => Some(CmdResult::PeditOp {
                        handle,
                        op: PeditOp::Reverse,
                    }),
                    "L" | "LTYPE" | "LTYPEGEN" => {
                        self.mode = Mode::AwaitLinetype;
                        Some(CmdResult::NeedPoint)
                    }
                    "U" | "UNDO" if self.undo_count > 0 => {
                        self.undo_count -= 1;
                        self.mesh_closed_history.pop();
                        if let Some(tangents) = self.tangent_history.pop() {
                            self.vertex_tangents = tangents;
                        }
                    ...
                    "J" | "JOIN" => {
                        self.mode = if self.multiple.is_empty() {
                            Mode::JoinGather(vec![handle])
                        } else {
                            Mode::MultipleJoin
                        };
                        Some(CmdResult::NeedPoint)
        ...
        if let Some(CmdResult::PeditOp { op, .. }) = &result {
            self.pending_tangents = self.mapped_tangents(op);
        }
        ...
        if let Mode::PolyTangent(index) = self.mode {
            ...
            let (normal, elevation, location) = match entity {
                EntityType::LwPolyline(p) => {
                    let Some(v) = p.vertices.get(index) else {
                        return CmdResult::NeedPoint;
                    };
                    (p.normal, p.elevation, [v.location.x, v.location.y])
                }
            ...
            let Some(local) = crate::entities::curve::ocs_plane(normal, elevation)
                .project([point.x, point.y, point.z])
            else {
                return CmdResult::NeedPoint;
            };
            let delta =
                cadkernel::geom2d::Vec2::new(local[0] - location[0], local[1] - location[1]);
            if !delta.length_squared().is_finite() || delta.length_squared() <= 1e-24 {
                return CmdResult::NeedPoint;
            }
            self.vertex_tangents
                .insert(index, delta.angle().rem_euclid(TAU));
            self.mode = Mode::PolyVertex(index);
            return CmdResult::NeedPoint;
        }
    ...
    fn on_enter(&mut self) -> CmdResult {
        ...
        match &mut self.mode {
            Mode::MultipleGather => {
                ...
                self.mode = if self.multiple.iter().any(|handle| {
                    self.info
                        .get(&handle.value())
                        .is_some_and(|info| info.convertible)
                }) {
            ...
            Mode::MultipleJoin => self
                .on_text_input(&self.join_fuzz.to_string())
                .unwrap_or(CmdResult::NeedPoint),
            Mode::JoinType => {
                self.mode = Mode::MultipleJoin;
                CmdResult::NeedPoint
            }
            Mode::PolyWidthStart(_, width) | Mode::PolyWidthEnd(_, width) => {
                let value = width.to_string();
                self.on_text_input(&value).unwrap_or(CmdResult::NeedPoint)
            }
            Mode::PolyVertex(_) | Mode::PolyRange { .. } => {
                self.on_text_input("N").unwrap_or(CmdResult::NeedPoint)
            }
            Mode::PolyMove(_) | Mode::PolyInsert(_) => {
                self.mode = Mode::Options;
                CmdResult::NeedPoint
            }
            Mode::AwaitLinetype => {
...
pub fn edit_vertex_range(
    ...
    macro_rules! edit {
        ($polyline:expr, $closed:expr, $variant:ident, $set_open:expr) => {{
            ...
            if first > last || last >= polyline.vertices.len() {
                return None;
            }
            if split && first == last && (first == 0 || last + 1 == polyline.vertices.len()) {
                return None;
            }
            if !split {
                if first == last {
                ...
                if $closed {
                    vertices.push(vertices.first()?.clone());
                }
    ...
    match entity {
        EntityType::LwPolyline(polyline) => edit!(
            polyline,
            polyline.is_closed,
            LwPolyline,
            |value: &mut acadrust::entities::LwPolyline| {
                value.is_closed = false;
            }
        ),
        EntityType::Polyline2D(polyline) => edit!(
            polyline,
            polyline.flags.is_closed(),
            Polyline2D,
            |value: &mut acadrust::entities::Polyline2D| {
                value.flags.set_closed(false);
            }
        ),
        _ => None,
    }
...
pub fn apply_pedit(entity: &mut EntityType, op: &PeditOp) -> bool {
    match op {
        PeditOp::Multiple(_, _) | PeditOp::JoinSelection(_, _, _) | PeditOp::VertexRange { .. } => {
            false
        }
                ...
                EntityType::LwPolyline(p) => {
                    ...
                    if p.constant_width != 0.0 {
                        for v in &mut p.vertices {
                            v.start_width = p.constant_width;
                            v.end_width = p.constant_width;
                        }
                        p.constant_width = 0.0;
                    }
                    p.vertices[*index].start_width = *start;
                    p.vertices[*index].end_width = *end;
                    true
                }
                EntityType::Polyline2D(p) => {
                    if *index >= p.vertices.len() {
                    ...
                    for v in &mut p.vertices {
                        if v.start_width == 0.0 {
                            v.start_width = p.start_width;
                        }
                        if v.end_width == 0.0 {
                            v.end_width = p.end_width;
                        }
                    }
                    p.start_width = 0.0;
                    p.end_width = 0.0;
        ...
        PeditOp::EditVertex {
            ...
            match entity {
                EntityType::LwPolyline(polyline) => {
                    let plane =
                        crate::entities::curve::ocs_plane(polyline.normal, polyline.elevation);
                    let Some(local) = plane.project([point.x, point.y, point.z]) else {
                        return false;
                    };
                    if *index >= polyline.vertices.len() {
                        return false;
                    }
                    if *insert {
                        polyline
                            .vertices
                            .insert(index + 1, LwVertex::new(Vector2::new(local[0], local[1])));
                    } else {
                        polyline.vertices[*index].location = Vector2::new(local[0], local[1]);
                    }
                    true
                }
                EntityType::Polyline2D(polyline) => {
                    ...
                    if *insert {
                        polyline.vertices.insert(
                            index + 1,
                            acadrust::entities::polyline::Vertex2D::new(location),
                        );
                    } else {
                        polyline.vertices[*index].location = location;
                    }
                    true
                }
                _ => false,
        ...
        PeditOp::Reverse => {
            let Some(reversed) = super::reverse::ReverseCommand::reversed(entity) else {
                return false;
            };
            *entity = reversed;
            true
        ...
        PeditOp::SetLinetypeGeneration(enabled) => match entity {
            ...
            EntityType::Polyline2D(polyline) => {
                ...
                if (bits & flag.bits() != 0) == *enabled {
                    return false;
                }
                polyline.flags =
                    acadrust::entities::polyline::PolylineFlags::from_bits(if *enabled {
                        bits | flag.bits()
                    } else {
                        bits & !flag.bits()
                    });
        ...
        PeditOp::Fit | PeditOp::FitWithTangents(_) => {
            let overrides = match op {
                PeditOp::FitWithTangents(values) => values.as_slice(),
                _ => &[],
            };
            let Some(fitted) = fit_entity(entity, overrides) else {
                return false;
            };
            *entity = fitted;
            true
        ...
        PeditOp::Decurve => {
            let Some(straight) = decurve_entity(entity) else {
                return false;
            };
            *entity = straight;
            true
        ...
        PeditOp::SetMeshSmooth {
            ...
            EntityType::PolygonMesh(mesh) => {
                ...
                if mesh.smooth_type != acadrust::entities::polygon_mesh::SurfaceSmoothType::NoSmooth
                {
...
fn fit_entity(entity: &EntityType, overrides: &[(usize, f64)]) -> Option<EntityType> {
    ...
    let mut polyline = match entity {
        ...
        EntityType::LwPolyline(p) => {
            ...
            result.common = p.common.clone();
            result.normal = p.normal;
            result.elevation = p.elevation;
            result.thickness = p.thickness;
            result.flags =
                PolylineFlags::from_bits((p.is_closed as u16) | if p.plinegen { 128 } else { 0 });
            result.vertices = p
                .vertices
                .iter()
                .map(|v| {
                    let mut vertex =
                        Vertex2D::new(Vector3::new(v.location.x, v.location.y, p.elevation));
                    vertex.start_width = if p.constant_width != 0.0 {
                        p.constant_width
                    } else {
                        v.start_width
                    };
                    vertex.end_width = if p.constant_width != 0.0 {
                        p.constant_width
                    } else {
                        v.end_width
                    };
    ...
    for vertex in &mut originals {
        if vertex.start_width == 0.0 {
            vertex.start_width = polyline.start_width;
        }
        if vertex.end_width == 0.0 {
            vertex.end_width = polyline.end_width;
        }
    }
    if originals.iter().any(|vertex| {
        !vertex.start_width.is_finite()
            || !vertex.end_width.is_finite()
    ...
    let points: Vec<_> = originals
        ...
        .collect();
    let directions: Vec<_> = originals
        .iter()
        .map(|v| (v.flags.bits() & 2 != 0).then(|| [v.curve_tangent.cos(), v.curve_tangent.sin()]))
        .collect();
    let fitted =
        cadkernel::geom2d::fit_arc_chain(&points, polyline.flags.is_closed(), &directions)?;
    ...
    for (i, fitted_vertex) in fitted.iter().enumerate() {
        ...
        vertex.location = Vector3::new(
            fitted_vertex.point[0],
            fitted_vertex.point[1],
            polyline.elevation,
        );
        vertex.bulge = fitted_vertex.bulge;
        let end_fraction = fitted
            .get(i + 1)
            .filter(|next| next.source == fitted_vertex.source)
            .map_or(1.0, |next| next.fraction);
        vertex.start_width = original.start_width
            + (original.end_width - original.start_width) * fitted_vertex.fraction;
        vertex.end_width =
            original.start_width + (original.end_width - original.start_width) * end_fraction;
        if fitted_vertex.inserted {
            vertex.flags = VertexFlags::EXTRA_VERTEX;
            vertex.id = 0;
        }
...
fn defining_vertices(
    polyline: &acadrust::entities::Polyline2D,
) -> Vec<acadrust::entities::Vertex2D> {
    polyline
        .vertices
        .iter()
        .enumerate()
        .filter(|(_, vertex)| vertex.flags.bits() & 1 == 0)
        .map(|(index, vertex)| {
            let mut original = vertex.clone();
            if let Some(knee) = polyline.vertices[index + 1..]
                .iter()
                .take_while(|next| next.flags.bits() & 1 != 0)
                .last()
            {
...
fn decurve_entity(entity: &EntityType) -> Option<EntityType> {
    let mut result = match entity {
        ...
        EntityType::Polyline2D(p) => {
            ...
            result.elevation = p.elevation;
            result.thickness = p.thickness;
            result.is_closed = p.flags.is_closed();
            result.plinegen = p.flags.bits() & 128 != 0;
            result.vertices = defining_vertices(p)
                .iter()
                .map(|v| {
                    let mut vertex = LwVertex::new(Vector2::new(v.location.x, v.location.y));
                    vertex.vertex_id = v.id;
                    vertex.start_width = if v.start_width == 0.0 {
                        p.start_width
                    } else {
                        v.start_width
                    };
                    vertex.end_width = if v.end_width == 0.0 {
                        p.end_width
                    } else {
                        v.end_width
                    };
    ...
    if let Some(width) = result.vertices.first().map(|v| v.start_width) {
        if result
            .vertices
            .iter()
            .all(|v| v.start_width == width && v.end_width == width)
        {
...
mod editing_tests {
    ...
    fn tangent_accepts_a_typed_angle_and_forwards_it_to_fit() {
        ...
        assert!(matches!(
            command.on_text_input("90"),
            Some(CmdResult::NeedPoint)
        ));
        assert!(matches!(
            command.on_text_input("X"),
            Some(CmdResult::NeedPoint)
        ));

        let Some(CmdResult::PeditOp {
            op: PeditOp::FitWithTangents(values),
            ..
    ...
    fn vertex_ranges_preserve_complete_endpoint_records() {
        ...
        let EntityType::LwPolyline(polyline) = &mut entity else {
            unreachable!()
        };
        ...
        let EntityType::LwPolyline(first) = &split[0] else {
            unreachable!()
        };
        let EntityType::LwPolyline(second) = &split[1] else {
            unreachable!()
        };
        ...
        let EntityType::LwPolyline(straightened) = &straightened[0] else {
            unreachable!()
        };
    ...
    fn fit_and_decurve_preserve_plane_identity_and_uniform_width() {
        ...
        let EntityType::Polyline2D(fitted) = &entity else {
            panic!("Fit must use the legacy fitted representation")
        };
        ...
        let EntityType::LwPolyline(straight) = &entity else {
            panic!("Decurve must restore a lightweight polyline")
        };
    ...
    fn join_fuzz_extends_only_supported_straight_terminals_within_the_limit() {
        ...
        assert!(
            join_selection_extend(&source, &candidates, 0.99, JoinType::Extend, None).is_none()
        );
        let (joined, consumed) =
            join_selection_extend(&source, &candidates, 1.0, JoinType::Extend, None).unwrap();
        ...
        assert!(join_selection_extend(&curved, &candidates, 2.0, JoinType::Extend, None).is_none());
...
pub fn join_selection_extend(
    source: &EntityType,
    candidates: &[(Handle, &EntityType)],
    fuzz: f64,
    kind: JoinType,
    connector_distance: Option<f64>,
    ...
    fn endpoint(entity: &EntityType, start: bool) -> Option<[[f64; 3]; 2]> {
        let (points, normal, elevation) = match entity {
            EntityType::LwPolyline(p) if !p.is_closed && p.vertices.len() >= 2 => {
                ...
                let segment = if start { 0 } else { n - 2 };
                if p.vertices[segment].bulge != 0.0 {
                    return None;
                }
    ...
    fn move_endpoint(entity: &mut EntityType, start: bool, point: [f64; 3]) -> Option<()> {
        match entity {
            ...
            EntityType::Polyline2D(p) => {
                ...
                p.vertices[index].location.x = local[0];
                p.vertices[index].location.y = local[1];
            }
            _ => return None,
        }
        Some(())
    ...
    loop {
        ...
        for (handle, candidate) in candidates {
            ...
            let mut joined = super::join::join_to_source(&result, &[(*handle, *candidate)])
                .map(|(entity, _)| entity);
            if joined.is_none()
                && fuzz > 0.0
                && fuzz.is_finite()
                && (kind == JoinType::Extend || connector_distance.is_some())
            {
                let a_ends = [false, true]
                    .into_iter()
                    .filter_map(|start| endpoint(&result, start).map(|points| (start, points)))
                    .collect::<Vec<_>>();
                let b_ends = [true, false]
                    .into_iter()
                    .filter_map(|start| endpoint(candidate, start).map(|points| (start, points)))
                    .collect::<Vec<_>>();
                let a_points = a_ends.iter().map(|(_, points)| *points).collect::<Vec<_>>();
                let b_points = b_ends.iter().map(|(_, points)| *points).collect::<Vec<_>>();
                if let Some((a_index, b_index, joint)) =
                    cadkernel::space::endpoint_join::closest_line_end_join(
                        &a_points,
                        &b_points,
                        fuzz,
                        connector_distance.unwrap_or(0.0),
                        kind,
                    )
                {
                    let (a_start, a_points) = a_ends[a_index];
                    let (b_start, b_points) = b_ends[b_index];
                    let mut a = result.clone();
                    let mut b = (*candidate).clone();
                    let joined_pair = if let Some(point) = joint {
                        if move_endpoint(&mut a, a_start, point).is_none()
                            || move_endpoint(&mut b, b_start, point).is_none()
                        {
                            continue;
                        }
                        ...
                        super::join::join_to_source(
                            &a,
                            &[(Handle::NULL, &connector), (*handle, &b)],
                        )
                        .filter(|(_, used)| used.contains(handle))
                    };
                    if let Some((entity, _)) = joined_pair {
                        joined = Some(entity);
                    }
                }
            }
...
pub fn selection_connector_distance(entities: &[(Handle, EntityType)], fuzz: f64) -> Option<f64> {
    ...
    for (_, entity) in entities {
        let (vertices, normal, z) = match entity {
            EntityType::LwPolyline(p)
                if !p.is_closed
                    && p.vertices.len() >= 2
                    && p.vertices.iter().all(|v| v.bulge == 0.0) =>
            {
                (
                    p.vertices
                        .iter()
    ...
    if curves
        .iter()
        .any(|(candidate, _)| normal.cross(*candidate).length() > 1e-9)
    {
        return None;
    }
    let world = curves
    ...
    let shared = crate::entities::curve::ocs_plane(
        Vector3::new(normal.x, normal.y, normal.z),
        cadkernel::space::Vec3::from(world[0]).dot(*normal),
    );
    if world
        .iter()
        .any(|point| !shared.contains(*point, tolerance))
    {
        return None;
    }
    let points = world
        .into_iter()
        .map(|point| shared.project(point))
