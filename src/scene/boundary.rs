...
impl Scene {
    pub(crate) fn replace_hatch_association(
        &mut self,
        handle: Handle,
        paths: Vec<acadrust::entities::BoundaryPath>,
    ) {
        let Some(EntityType::Hatch(hatch)) = self.document.get_entity_mut(handle) else {
            return;
        };
        hatch.paths = paths;
                ...
                let sources: rustc_hash::FxHashMap<_, _> = path
                    ...
                    .filter_map(|source| {
                        ...
                        let geometry = self
                            ...
                            .get_entity(*source)
                            .and_then(|entity| {
                                entity_boundary_source_on_plane(entity, plane, WELD_TOLERANCE)
                            })
