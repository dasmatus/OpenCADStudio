...
pub fn profile_geometry(entity: &EntityType) -> Option<(Plane, Vec<Vec<Curve>>, bool)> {
    ...
    let embedded = match entity {
        EntityType::Region(region) => Some((
            EmbeddedEntity::Region(region.clone()),
...
fn eligible(scene: &Scene, entity: &EntityType) -> bool {
    ...
    !common.invisible
        ...
        && scene.belongs_to_visible_block(
            common.handle,
            common.owner_handle,
            scene.interaction_block_handle(),
        )
        && !crate::scene::annotative::annotative_offscale_for(
            &scene.document,
            common,
            scene.displayed_annotation_scale_handle(),
...
fn working_plane(plane: Plane) -> WorkingPlane {
    WorkingPlane::new(
        DVec3::from_array(plane.origin),
        DVec3::from_array(plane.x_axis),
...
fn boundary_sources(scene: &Scene, plane: WorkingPlane) -> FxHashMap<Handle, BoundarySource> {
    ...
    scene
        .document
        .entities()
        .filter(|entity| eligible(scene, entity))
        .filter_map(|entity| {
            let (source, loops, _) = profile_geometry(entity)?;
            let curves = loops
                .iter()
                .flat_map(|ring| curves_on_plane(source, ring, plane))
                .collect();
...
pub(crate) fn selected_rings(
    sources: &FxHashMap<Handle, BoundarySource>,
    point: [f64; 2],
    ...
    for (index, ring) in rings.iter().enumerate() {
        if index != outer
            && depths.get(index) == depths.get(outer).map(|depth| depth + 1).as_ref()
            && ring
                .first()
                .is_some_and(|point| contains(boundary, *point, tolerance))
        {
...
pub(crate) fn region_from_loops(loops: &[Vec<Curve>], plane: WorkingPlane) -> Option<EntityType> {
    let kernel_plane = Plane::from_axes(
...
fn face_at_point(
    scene: &Scene,
    point: DVec3,
    preferred: Option<Handle>,
) -> Option<(Handle, FaceKey, &Body)> {
...
fn profile_owner(scene: &Scene, plane: WorkingPlane, anchor: DVec3) -> Option<(Handle, DVec3)> {
    ...
    for handle in handles {
        ...
        for face in body.face_keys() {
            let Some(profile) = brep::planar_face_profile(body, face) else {
                continue;
            };
...
pub fn resolve_target(
    ...
    if let Some(handle) = handle {
        ...
        if let Some((source_plane, _, closed)) = profile_geometry(entity) {
            ...
            let attached = closed
                .then(|| profile_owner(scene, profile_plane, anchor))
                .flatten();
            let owner = attached.map(|(owner, _)| owner);
            let direction = attached
                .map(|(_, outward)| outward)
        ...
        if !matches!(entity, EntityType::Solid3D(_)) {
            return Err(
                "PRESSPULL: select a planar curve, bounded area or planar solid face.".to_owned(),
            );
        }
    }
        ...
        if !offset {
            ...
            if let Some(rings) = selected_rings(&sources, [local.x, local.y]) {
                let bounded_by_drawing = rings.iter().any(|ring| {
                    ring_source_handles(ring, &sources)
                        .into_iter()
                        .any(|source| source != owner)
                });
                if bounded_by_drawing {
                    let entity = boundary_region(&sources, &rings, face_plane).ok_or(
                        "PRESSPULL: the selected boundary could not be reconstructed exactly.",
                    )?;
                    return Ok(PresspullTarget {
                        kind: PresspullTargetKind::Profile {
                            entity,
    ...
    if handle.is_some() {
        return Err(
            "PRESSPULL: select inside a trimmed planar face, outside its holes.".to_owned(),
        );
    }
...
pub fn preview_wires(
    target: &PresspullTarget,
    distance: f64,
    color: [f32; 4],
    isolines: usize,
) -> Vec<WireModel> {
    ...
    if let PresspullTargetKind::Face {
        body,
        face,
        offset: true,
        ..
    ...
    let geometry = match &target.kind {
        ...
        PresspullTargetKind::Face { body, face, .. } => {
            ...
            brep::planar_face_profile(body, *face).map(|profile| {
                (
                    profile.plane,
                    profile.loops,
    ...
    let Some((plane, loops, direction)) = geometry else {
        return Vec::new();
    };
    ...
    for ring in loops {
        ...
        let Some(body) = brep::extrude_surface(plane, &ring, direction) else {
            continue;
        };
...
fn preview_body_wires(body: &Body, color: [f32; 4], isolines: usize) -> Vec<WireModel> {
    let wireframe = brep::mesh::tessellate_wireframe(
        body,
