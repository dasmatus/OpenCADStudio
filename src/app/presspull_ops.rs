...
use acadrust::objects::{
    DynamicBlockData, ObjectType, SolidHistoryNodeBase, SolidHistoryOperation, SolidHistorySweep,
};
...
impl super::OpenCADStudio {
    ...
    fn prepare_presspull(
        &self,
        targets: &[PresspullTarget],
        distance: f64,
    ) -> Result<Vec<PreparedPresspull>, String> {
        ...
        let mut results: Vec<(Option<Handle>, EntityType, Body, SolidHistoryOperation)> =
            Vec::new();
        for target in targets {
            let owner = match &target.kind {
                PresspullTargetKind::Profile {
                    ...
                    if let Some(source) = source {
                        if scene.is_layer_locked(*source)
                            || scene.document.get_entity(*source) != Some(entity)
                        {
                            return Err(
                                "PRESSPULL: a selected source changed. Select it again.".into()
                            );
                        }
                    }
                    *owner
            ...
            let previous =
                owner.and_then(|handle| results.iter().position(|result| result.0 == Some(handle)));
            ...
            let (body, history, mut entity) = match &target.kind {
                PresspullTargetKind::Profile { entity, .. } => {
                    ...
                    if owner.is_some() {
                        ...
                        let profile = PlanarFaceProfile {
                            plane,
                            loops,
                            outward: target.direction.to_array(),
                        };
                        let body = brep::presspull_region(current, &profile, distance).ok_or(
                            "PRESSPULL: the region could not be added to or cut from the solid.",
                        )?;
                        ...
                        let body = presspull_model::extrusion_body(entity, direction.to_array())
                            .ok_or(
                                "PRESSPULL: the profile could not be extruded with this height.",
                            )?;
                        let history = extrusion_history(entity, direction, target.anchor).ok_or(
                            "PRESSPULL: the editable extrusion profile could not be retained.",
                        )?;
                        let entity = if closed {
                            ...
                            if let SolidHistoryOperation::Extrusion(record) = &history {
                                surface.surface_data =
                                    solid_history::extrusion_surface_data(record).ok_or(
                                        "PRESSPULL: the surface parameters could not be retained.",
                                    )?;
                            }
                ...
                PresspullTargetKind::Face { face, offset, .. } => {
                    let current =
                        current.ok_or("PRESSPULL: the original solid could not be restored.")?;
            ...
            let sat = crate::scene::convert::acis_export::solid_to_sat(&body).ok_or(
                "PRESSPULL: the result could not be encoded. The original objects were retained.",
            )?;
            if owner.is_none() {
                entity.common_mut().plotstyle_flags = 2;
            }
            match &mut entity {
    ...
    pub(super) fn presspull_apply(&mut self, targets: Vec<PresspullTarget>, distance: f64) {
        ...
        for result in prepared {
            let handle = if let Some(handle) = result.owner {
                ...
                self.commit_entity_handle(result.entity)
                    .expect("validated solid/surface insertion")
            };
            self.tabs[i]
                .scene
                .create_solid_history(handle, result.history);
            ...
            if result.owner.is_none() {
                ...
                if let Some(graph) = document.solid_history_graph(handle) {
                    if let Some(ObjectType::DynamicBlock(object)) =
                        document.objects.get_mut(&graph.root)
                    {
            ...
            self.tabs[i]
                .scene
                .register_prepared_solid_model(handle, result.body, result.display);
            handles.push(handle);
        }
        ...
        self.command_line.push_output(
            crate::tf!(
                "PRESSPULL: {count} object(s) updated.",
                count = handles.len()
            )
            .as_ref(),
        );
